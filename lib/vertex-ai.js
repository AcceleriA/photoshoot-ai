 ...
 * TTL de 25 minutes (les tokens Google expirent à 60min)
 */
 ...
 * @param {string[]} referenceImagesBase64 - Array d'images sources en base64 JPEG
 * @param {string} accessToken - Token d'accès Google Cloud
 */
 ...
 * Si le premier essai échoue, on retente une fois. Pas plus.
 */
async function generateWithRetry(prompt, referenceImagesBase64, accessToken) {
  try {
    ...
    console.warn(`Premier essai échoué, retry dans 2s: ${err.message}`);
...
export async function generateImages(prompt, referenceImagesBase64, sampleCount = 4) {
  ...
  if (images.length === 0) {
    // Toutes les générations ont échoué : remonter le message le plus pertinent
